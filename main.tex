...
3 and 8 ms \marginpar{Citation}, which gives the recording amplifier
...
\begin{figure}
  ...
  \caption{An example of a clear response in HVC.  The horizontal axis is time, in milliseconds relative to the onset of a stimulation pulse.  Here, the stimulation is a 400-$\mu$s biphasic pulse of 3 $\mu$A, in which voltage peaked at 1.6 V.  Stimulation was repeated 20 times, with each response aligned to its respective pulse and shown as a separate colour.  Various response activity can be seen, but the most pronounced is at 6.2 ms post-onset.  {\em Use the top figure?  It only shows one channel, and it's recorded on the Intan, which I was using while the HVC signals were still really pretty.  That means a lot worse amplifier settling, so it's not the best image, but the response is much cleaner than later ones made with the TDT.}}
