...
The first DBS systems deliver some kind of stimulation continuously.  Recently, interest in using biological feedback has grown, but due to the size of the electrodes, current systems rely on slow-timescale local field potentials (LFP) and other macroscopic measures of outcome such as accelerometers for feedback control \cite{Afshar2012closedloop}.  \cite{McIntyre2015} notes that there is currently little evidence that feedback-sensitive ``closed-loop'' systems produce superior clinical outcomes, but that this may be due to an incomplete understanding of the brain.\marginpar{Power use may be improved\dots}

Current steering: \cite{Chaturvedi2012} uses 3D finite element analysis to predict current steering trajectory\marginpar{Verification??} They built a model based on magnetic resonance imaging (MRI) and diffusion tensor imaging (DTI) \cite{Alexander2007dti}




Our electrodes offer the ability to record and stimulate precise areas at almost the same time.
...
3 and 8 ms \marginpar{Tim: citation for 3--8 ms?}, which gives the recording amplifier
time to settle before measuring the response.\marginpar{But why
  antidromic?}
...
\fig{fig:clear_hvc_response} shows an example of a pronounced HVC response to stimulation in Area X.  In order to detect this signal, we measure the cross-correlation between the recorded response for each pulse in a train and each other with a maximum lag of 50 $\mu$s, which provides a robust way of separating neural response from noise.  Because the HVC amplifier's response to the transient stimulation pulse in Area X can persist into the time of interest (3--8 ms), each response is first de-trended using a maximum-likelihood fit over the region 2--25 ms using an eigth order Fourier Series, which removes post-stimulation decay while leaving intact spike-sized signals.  We found this to produce fewer artifacts than the more conventional approach of bandpass-filtering the signal, given the large stimulation artifact.  The cross-correlation threshold above which a response is identified is chosen by visual inspection.
