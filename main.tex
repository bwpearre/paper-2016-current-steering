...
of interest, but we feel that this experiment nonetheless hints at the
...
when voltage exceeds the hydrolysis point for water (around 1.3
...
the recording electrode, for some time post-stimulation. Artifacts
were visible in the recording traces for 1--3 ms
post-stimulus.\marginpar{Intan} Responses to stimulation can be
detected outside of the saturation window.  A common technique for
locating HVC in the zebra finch involves implanting a stimulating
electrode in Area X and looking for a an antidromic response, which is
visible in HVC and not in the surrounding tissue
\cite{Swadlow1998antidromic,Hahnloser2002sparse}.  Since the two brain
areas are separated by about 5 mm, this response occurs 3--8 ms after
stimulation\marginpar{Tim: citation for 3--8 ms?}, which gives the
recording amplifier time to settle before measuring the
response.\marginpar{But why antidromic? Could one go forward? Or would
  that be too risky to HVC if that's the target of study?}
...
easily seen in several channels after a year.\marginpar{I don't have
  ...
  bird.}  \fig{fig:XSpikeRecording} shows recordings a year after
