...
patterns to target the desired target.\marginpar{Lots more references
...
for clinician-assisted hillclimbing search for effective stimulation
in multielectrode arrays in the spinal cord, and
\cite{parramon2011current_steering_optimisation} develops
another\marginpar{Verify this. Internet's down as I write\dots}
...
measures of outcome such as surface electromyography\marginpar{Also:
  Basu, ``Pathological tremor prediction using surface electromyogram
  and acceleration''} and accelerations
\cite{Afshar2012closedloop}\marginpar{Great review:
  \cite{Priori2015review}}. The addition of feedback from a second
electrode in a related brain region is effective in ameliorating
Parkinsonian symptoms in monkeys \cite{Rosin2011adbs}, in which the
use of tungsten microelectrodes demonstrated the effectiveness of
feedback that responds to individual spikes rather than just LFPs.
...
both impedance and charge injection capacity.\marginpar{Confirm
  ...
  anywhere? But more than that, since we only did this with one
  chronic electrode (and the short-term birds), is it even worth
  listing here? Or should I just mention it in Methods?} Second, we
introduce a novel current-steering technique, and provide preliminary
evidence that appropriate choice of current-steering configuration can
be used to reduce stimulation voltage.
...
amplifier or an Intan RZ2000\marginpar{Model #? Also depending on
  which recording to show in \fig{fig:clear_hvc_response}}.  The
Plexon self-monitoring channels were recorded on a National
...
Programming each of the stimulator's 16 channels for one pulse train and running the programme (10 pulses at 25 Hz) requires a little over 2 seconds. A threshold scan requires, on average, around 15 pulse trains, and so generally takes a bit more than 30 seconds. Likewise, a voltage scan requires delivering one pulse train per active electrode, for a total of about 30 seconds.\marginpar{This timing is after I did some speedup hacking, but the experiments presented in this paper were pre-speedup (about half as fast). Does it matter?}
...
\begin{figure}
  ...
  \caption{An example of a strong response in HVC.  {\bf Top:} The
    ...
    recorded on TDT.  200 $\mu$s, 6.94 $\mu$A, 1.2 V peak.{\em Use the
...
search would in addition be easier to describe, this ad-hoc approach
...
More telling of the potential of these electrodes to detect spikes is
...
easily seen in several channels after a year.\marginpar{I don't have
  recording-only data from early on.}  \fig{fig:XSpikeRecording} shows
recordings a year after implantation for the most stable electrodes of
the 16.
...
others.  From the $2^{11}$\marginpar{Explain why this number. Why only
  one bird?} possible CSCs we chose 32 (30 randomly, and the two that
...
response.  When this current was found, we performed a voltage scan to
find the maximum potential on any electrode.  We tested each CSC five
times on an anesthetised bird.  Results are shown in
\fig{fig:VoltageVsCSC}.  The best CSCs resulted in a maximum voltage
of around 1~V, while the worst were over 2.5~V.  Perhaps surprisingly, the
CSCs that sent identical pulses to all 11 electrodes were among the
worst performers, with our simple search revealing CSCs that kept
voltages far lower.
...
spike? This requires search over current delivered to each group in
$n$ in order to control which neurons are recruited, and timing of
stimulation delivery to each group in $n$, so that appropriate
downstream neurons are reliably triggered.  Different values of
current and timing delivered into the $n$ groups may trigger different
downstream neurons, so one possible search problem is finding as many
different downstream neurons as possible.
...
different network-scale dynamics. (\cite{Jepson2014steering_in_retina}
...
clinical outcomes require about 20 hours\marginpar{Now where did I
  read this\dots CITATION NEEDED?} of hand-tuning, involving multiple
patient visits to a clinic.\marginpar{This has nothing to do with our paper. Learning is not addressed here. But I could mention it as a possibility. Should I? Or leave it out?}
